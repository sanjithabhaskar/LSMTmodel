...
# Convert predicted prices into a DataFrame with datetime index
predicted_df = pd.DataFrame(predicted_prices, index=future_dates, columns=['Predicted'])

# Decompose the predicted time series
decomposition = seasonal_decompose(predicted_df['Predicted'], model='multiplicative', period=252)

# Approximate cyclical component
# Avoid division by zero by dropping NaNs or infs from beginning
cyclical = predicted_df['Predicted'] / (decomposition.trend * decomposition.seasonal)
cyclical = cyclical.dropna()

# Plot all components including cyclical
plt.figure(figsize=(14, 12))

plt.subplot(511)
plt.plot(predicted_df['Predicted'], label='Original (Predicted)')
plt.legend(loc='upper left')
plt.title('Original (Predicted) Data')
...
plt.subplot(514)
plt.plot(cyclical, label='Cyclical (Smoothed)', color='purple')
plt.legend(loc='upper left')
plt.title('Cyclical Component (Approximated)')
